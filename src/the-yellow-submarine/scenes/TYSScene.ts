 ...
 * represent the different layers in the scene, however, it is generally bad practice to use
 * Typescripts enums. As an alternative, I'm using a const object.
 * 
 * @author McKenna-Weaver-Grandison-Walsh
 * 
...
export default class TYSScene extends Scene {
    ...
    public static MINE_PATH = "game_assets/spritesheets/SpikyMineThing.json"
	...
	public override loadScene(){
		...
		this.load.spritesheet(TYSScene.MINE_KEY, TYSScene.MINE_PATH);
	...
	protected initPlayer(): void {
		...
		this.player = this.add.animatedSprite(TYSScene.PLAYER_KEY, TYSLayers.PRIMARY);
	...
	/**
	 * Initializes the UI for the scene.
	 * 
	 * @remarks
	 * 
	...
	protected initUI(): void {
		// UILayer stuff
		this.addUILayer(TYSLayers.UI);

		// HP Label
		this.healthLabel = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(50, 50), text: "HP "});
		this.healthLabel.size.set(300, 30);
		this.healthLabel.fontSize = 24;
		this.healthLabel.font = "Courier";

		// Air Label
		this.airLabel = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(50, 100), text: "Air"});
		this.airLabel.size.set(300, 30);
		this.airLabel.fontSize = 24;
		this.airLabel.font = "Courier";

		// Charge Label
		this.chrgLabel = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(475, 50), text: "Lasers"});
		...
		for (let i = 0; i < this.chrgBarLabels.length; i++) {
			...
			this.chrgBarLabels[i] = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: pos, text: ""});
		...
		// HealthBar
		this.healthBar = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(225, 50), text: ""});
		this.healthBar.size = new Vec2(300, 25);
		this.healthBar.backgroundColor = Color.GREEN;

		// AirBar
		this.airBar = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(225, 100), text: ""});
		this.airBar.size = new Vec2(300, 25);
		this.airBar.backgroundColor = Color.CYAN;

		// HealthBar Border
		this.healthBarBg = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(225, 50), text: ""});
		this.healthBarBg.size = new Vec2(300, 25);
		this.healthBarBg.borderColor = Color.BLACK;

		// AirBar Border
		this.airBarBg = <Label>this.add.uiElement(UIElementType.LABEL, TYSLayers.UI, {position: new Vec2(225, 100), text: ""});
	...
	protected initBackground(): void {
		this.bg1 = this.add.sprite(TYSScene.BACKGROUND_KEY, TYSLayers.BACKGROUND);
		this.bg1.scale.set(1.5, 1.5);
		this.bg1.position.copy(this.viewport.getCenter());

		this.bg2 = this.add.sprite(TYSScene.BACKGROUND_KEY, TYSLayers.BACKGROUND);
	...
	protected initObjectPools(): void {
		...
		for (let i = 0; i < this.bubbles.length; i++) {
			this.bubbles[i] = this.add.graphic(GraphicType.RECT, TYSLayers.PRIMARY, {position: new Vec2(0, 0), size: new Vec2(50, 50)});
		...
		for (let i = 0; i < this.mines.length; i++){
			this.mines[i] = this.add.animatedSprite(TYSScene.MINE_KEY, TYSLayers.PRIMARY);
		...
		for (let i = 0; i < this.lasers.length; i++) {
			this.lasers[i] = this.add.graphic(GraphicType.RECT, TYSLayers.PRIMARY, {position: Vec2.ZERO, size: Vec2.ZERO})
	 ...
	 * 
	 * @see TYSEvents.PLAYER_MINE_COLLISION the event to be fired when a collision is detected
	...
	public handleMinePlayerCollisions(): number {
		...
		for (let mine of this.mines) {
			if (mine.visible && this.player.collisionShape.overlaps(mine.collisionShape)) {
				this.emitter.fireEvent(TYSEvents.PLAYER_MINE_COLLISION, {id: mine.id});
				collisions += 1;
			}
		}	
		return collisions;
		...
		if (laser.visible) {
			for (let mine of mines) {
				if (mine.collisionShape.overlaps(laser.collisionShape)) {
					this.emitter.fireEvent(TYSEvents.LASER_MINE_COLLISION, { mineId: mine.id, laserId: laser.id });
					collisions += 1;
				}
			}
		}
